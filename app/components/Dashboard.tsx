...
interface BrandProduct {
  rank: number; name: string; price: string;
  original_price: string | null; badges: string[]; reviews: string;
}
interface BrandPromo { name: string; desc: string; highlight: string; }
interface BrandPoint { title: string; desc: string; }
interface BrandData {
  summary: string;
  tags: string[];
  stats: { product_count: number; promo_count: number; avg_rating: number };
  best_products: BrandProduct[];
  promotions: BrandPromo[];
  marketing_points: BrandPoint[];
  fetchedAt: string;
  error?: string;
}

const BADGE_STYLE: Record<string, React.CSSProperties> = {
  SALE: { background: 'rgba(239,68,68,0.08)', color: '#ef4444' },
  NEW:  { background: 'rgba(59,130,246,0.08)', color: '#3b82f6' },
  HOT:  { background: 'rgba(249,115,22,0.08)', color: '#f97316' },
  BEST: { background: KEY_BG, color: KEY },
};
...
function BrandSection() {
  ...
  const skel = (w: string, h: string) => (
    <div style={{ width: w, height: h, borderRadius: 6, ...shimmerStyle }} />
  );

  return (
    <div id="sec-brand">
      <div className="flex items-center gap-3 mb-5">
        <h2 className="text-sm font-semibold tracking-widest uppercase whitespace-nowrap" style={{ color: KEY }}>구스초이 리포트</h2>
        <div className="flex-1 h-px bg-black/6" />
        <span className="text-xs text-black/25 whitespace-nowrap">goosechoi.com · Gemini AI</span>
      </div>
      ...
      {loading && (
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-3">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="rounded-xl border border-black/6 p-4 space-y-2">
                {skel('40%', '28px')} {skel('60%', '14px')} {skel('50%', '12px')}
              </div>
            ))}
          </div>
          <div className="rounded-xl border border-black/6 p-4 space-y-2">
            {skel('100%', '14px')} {skel('85%', '14px')} {skel('70%', '14px')}
          </div>
        </div>
      )}

      {/* API 키 없음 */}
      {!loading && data?.error === 'no_key' && (
        <p className="text-xs text-black/30 py-4">
          GEMINI_API_KEY가 설정되지 않았습니다. .env.local을 확인하세요.
        </p>
      )}

      {/* 오류 */}
      {!loading && data?.error && data.error !== 'no_key' && (
        <p className="text-xs text-black/30 py-4">브랜드 분석을 불러오지 못했습니다.</p>
      )}
      ...
      {!loading && data && !data.error && (
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs"
            ...
            <span className="font-semibold" style={{ color: KEY }}>분석 완료</span>
            <span className="text-black/40 ml-1">Gemini 2.0 Flash</span>
            <span className="ml-auto text-black/30">{fmtKst(data.fetchedAt)}</span>
          </div>

          {/* 핵심 지표 */}
          <div className="grid grid-cols-3 gap-3">
            {[
              { value: data.stats.product_count, label: '노출 상품 수', sub: '구스초이 기준' },
              { value: data.stats.promo_count,   label: '진행 프로모션', sub: '현재 시즌' },
              { value: data.stats.avg_rating,    label: '평균 평점',     sub: '베스트 상품' },
            ].map(({ value, label, sub }) => (
              <div key={label} className="rounded-xl border border-black/6 px-4 py-3.5">
                <p className="text-2xl font-bold text-black">{value}</p>
                <p className="text-xs text-black/40 mt-0.5">{label}</p>
                <p className="text-xs mt-1 font-medium" style={{ color: KEY }}>{sub}</p>
              </div>
            ))}
          </div>
          ...
          <div className="rounded-xl border border-black/6 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-black/5">
              <span className="text-xs font-semibold text-black/40 tracking-widest uppercase">AI 브랜드 인사이트</span>
              <span className="text-xs font-bold px-2 py-0.5 rounded-full"
                style={{ background: KEY_BG, color: KEY, border: `1px solid ${KEY_BORDER}` }}>Gemini</span>
            </div>
            <div className="px-4 py-4">
              <p className="text-sm leading-relaxed text-black/70 mb-3">{data.summary}</p>
              <div className="flex flex-wrap gap-1.5">
                {data.tags.map(t => (
                  <span key={t} className="text-xs px-2.5 py-1 rounded-full border border-black/7 text-black/45 bg-black/[0.02]">{t}</span>
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="rounded-xl border border-black/6 overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-black/5">
                <span className="text-xs font-semibold text-black/40 tracking-widest uppercase">베스트 상품</span>
                <span className="text-xs text-black/25">판매량 기준</span>
              </div>
              <div>
                {data.best_products.map(p => (
                  <div key={p.rank}
                    className="flex items-center gap-3 px-4 py-3 border-b border-black/4 last:border-0 hover:bg-black/[0.01] transition-colors">
                    <span className="text-sm font-black w-4 shrink-0" style={{ color: KEY }}>{p.rank}</span>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold text-black/80 truncate mb-1">{p.name}</p>
                      <div className="flex flex-wrap gap-1 items-center">
                        {p.badges.map(b => (
                          <span key={b} className="text-[10px] font-bold px-1.5 py-0.5 rounded"
                            style={BADGE_STYLE[b] ?? BADGE_STYLE.BEST}>{b}</span>
                        ))}
                        {p.reviews && (
                          <span className="text-[10px] text-black/30">리뷰 {p.reviews}</span>
                        )}
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      ...
                      {p.original_price && (
                        <p className="text-[10px] text-black/30 line-through">{p.original_price}</p>
                      )}
                    </div>
                  </div>
                ))}
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="rounded-xl border border-black/6 overflow-hidden">
                <div className="px-4 py-3 border-b border-black/5">
                  <span className="text-xs font-semibold text-black/40 tracking-widest uppercase">진행 중 프로모션</span>
                </div>
                <div className="grid grid-cols-2 gap-2 p-3">
                  {data.promotions.map(pr => (
                    <div key={pr.name} className="rounded-lg border border-black/6 px-3 py-2.5 relative overflow-hidden">
                      <div className="absolute left-0 top-0 bottom-0 w-0.5" style={{ background: KEY }} />
                      <p className="text-xs font-bold mb-0.5 pl-1">{pr.name}</p>
                      <p className="text-[10px] text-black/40 mb-1.5 pl-1">{pr.desc}</p>
                      <p className="text-base font-black pl-1" style={{ color: KEY }}>{pr.highlight}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="rounded-xl border border-black/6 overflow-hidden">
                <div className="flex items-center justify-between px-4 py-3 border-b border-black/5">
                  <span className="text-xs font-semibold text-black/40 tracking-widest uppercase">마케팅 포인트</span>
                  <span className="text-xs font-bold px-2 py-0.5 rounded-full"
                    style={{ background: KEY_BG, color: KEY, border: `1px solid ${KEY_BORDER}` }}>AI 추출</span>
                </div>
                <div className="divide-y divide-black/4">
                  {data.marketing_points.map(pt => (
                    <div key={pt.title} className="px-4 py-3">
                      <p className="text-xs font-bold mb-0.5">{pt.title}</p>
                      <p className="text-[11px] text-black/45 leading-relaxed">{pt.desc}</p>
                    </div>
                  ))}
                </div>
              </div>
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-8 sm:space-y-10">
        ...
        {/* 구스초이 리포트 */}
        <section className="border border-black/6 rounded-2xl px-5 py-4">
          <BrandSection />
        </section>
