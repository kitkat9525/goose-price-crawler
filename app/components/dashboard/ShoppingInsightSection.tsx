...
const BAR_COLORS = ['#111111', '#888888'];
...
function HBarChart({ data, labelMap, highlightMax = false }: {
    ...
    <div className="flex flex-col gap-2">
      {mapped.map((d, i) => {
        const color = highlightMax
          ? (i === maxIdx ? '#111' : 'rgba(0,0,0,0.12)')
          : BAR_COLORS[i % BAR_COLORS.length];
        return (
          <div key={d.name} className="flex items-center gap-2">
            <span className="text-xs text-black/45 shrink-0" style={{ width: 44, textAlign: 'right' }}>{d.name}</span>
            <div className="flex-1 h-5 bg-black/[0.06] overflow-hidden">
              <div
                className="h-full flex items-center transition-all duration-500"
                style={{ width: `${d.ratio}%`, backgroundColor: color }}
              >
                {d.ratio >= 12 && (
                  <span className="text-[10px] font-bold text-white pl-2">{d.ratio}%</span>
                )}
              </div>
            </div>
            {d.ratio < 12 && (
              <span className="text-[10px] font-bold text-black/40 shrink-0">{d.ratio}%</span>
            )}
          </div>
        );
      })}
...
export function ShoppingInsightSection() {
  ...
  return (
    <section id="sec-insight">
      ...
      {loading && (
        <div className="border border-black/8 flex items-center justify-center py-12 text-sm text-black/30">
          불러오는 중...
        </div>
      )}

      {!loading && (!data || data.source === 'unavailable') && (
        <div className="border border-black/8 px-5 py-10 text-center">
          {data?.error === 'naver_key_missing'
            ? <>
                <p className="text-sm font-medium text-black/50">네이버 API 키가 설정되지 않았습니다</p>
                <p className="text-xs text-black/30 mt-1">
                  <code className="bg-black/5 px-1">NAVER_CLIENT_ID</code> /{' '}
                  <code className="bg-black/5 px-1">NAVER_CLIENT_SECRET</code> 필요
                </p>
              </>
      ...
      {!loading && data?.source === 'live' && (
        <div className="border border-black/8 overflow-hidden">
            ...
            {INSIGHT_KEYWORDS.map(k => (
              <button
                ...
                style={{
                  ...
                  color: active === k ? '#111' : 'rgba(0,0,0,0.35)',
                }}
              >
                {k}
              </button>
          ...
          <div className="px-5 pt-5 pb-3">
            <p className="text-xs font-bold text-black/35 uppercase tracking-widest mb-3">월별 클릭 트렌드</p>
            {chartData.length === 0
              ...
              : (
                <ResponsiveContainer width="100%" height={200}>
                  <LineChart data={chartData} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                    ...
                    />
                    <Line type="monotone" dataKey="ratio" stroke="#111" strokeWidth={2}
                      dot={{ r: 4, fill: '#111', strokeWidth: 0 }} activeDot={{ r: 5, fill: '#111' }}
                    />
                  </LineChart>
                </ResponsiveContainer>
              )
          ...
          <div className="grid grid-cols-2 border-t border-black/5">
            <div className="px-5 py-4 border-r border-black/5 flex flex-col gap-5">
              <div>
                <p className="text-xs font-bold text-black/35 uppercase tracking-widest mb-3">기기별</p>
                <HBarChart data={deviceData} labelMap={DEVICE_LABEL} />
              </div>
              <div>
                <p className="text-xs font-bold text-black/35 uppercase tracking-widest mb-3">성별</p>
                <HBarChart data={genderData} labelMap={GENDER_LABEL} />
              </div>
            </div>
            <div className="px-5 py-4">
              <p className="text-xs font-bold text-black/35 uppercase tracking-widest mb-3">연령별</p>
              <HBarChart data={ageData} labelMap={AGE_LABEL} highlightMax />
            </div>
          </div>
