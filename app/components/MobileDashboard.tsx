...
// ── 탭 정의 ───────────────────────────────────────────────────
type Tab = 'market' | 'news' | 'shopping' | 'sns';
const TABS: { id: Tab; label: string; icon: string }[] = [
  { id: 'market',   label: '시세',   icon: '📊' },
...
// ── 구분선 ────────────────────────────────────────────────────
function Divider({ margin = '0 0 20px' }: { margin?: string }) {
...
function MagCard({ cat, cur, fx }: { cat: CategoryPrices; cur: Currency; fx: FxRates }) {
  ...
  return (
    <div style={{ border: '1px solid #ebebeb', padding: '18px 16px' }}>
      <p style={{ fontSize: 9, fontWeight: 700, letterSpacing: 1.5, color: 'rgba(17,17,17,0.28)', marginBottom: 4, textTransform: 'uppercase' }}>
        {cat.nameKr}
      </p>
      ...
      {price != null ? (
        <>
          <p style={{ fontSize: 32, fontWeight: 900, letterSpacing: -1.5, lineHeight: 1, color: '#111' }}>
            {fmtPrice(price, cur)}
            <span style={{ fontSize: 12, color: 'rgba(17,17,17,0.3)', fontWeight: 400, marginLeft: 6 }}>/kg</span>
          </p>
          {diff != null && (
    ...
    <div ref={scrollRef} style={{ display: 'flex', gap: 12, overflowX: 'auto', scrollbarWidth: 'none', WebkitOverflowScrolling: 'touch' } as React.CSSProperties}>
      {items.map((item, i) => (
        <a key={`${item.link}-${i}`} href={item.link} target="_blank" rel="noopener noreferrer"
          ...
          <p style={{ fontSize: 11, fontWeight: 400, lineHeight: 1.4, color: '#111', marginBottom: 4,
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        {items.map(item => (
          <a key={item.videoId} href={`https://www.youtube.com/watch?v=${item.videoId}`} target="_blank" rel="noopener noreferrer"
            ...
            <p style={{ fontSize: 11, fontWeight: 400, lineHeight: 1.45, color: '#111',
              display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' } as React.CSSProperties}>
              {item.title}
            </p>
          </a>
    ...
    <div style={{ flex: 1, padding: '14px 16px', background: '#f8f8f8' }}>
      ...
      {price != null ? (
        <>
          ...
          {diff != null && <span style={{ display: 'inline-block', fontSize: 9, fontWeight: 700, padding: '2px 7px', borderRadius: 2, background: '#ebebeb', marginTop: 5, color: '#111' }}>{down ? '▼' : '▲'} {fmtPrice(diff, currency)}</span>}
        </>
...
// ── 탭 컨텐츠: 시세 ───────────────────────────────────────────
function MarketTab({ data, currency, saveCurrency, cfdData, cfdStd, cfdLoading, switchStd, eurKrw, usdKrw, cnyKrw, lastUpdated }: {
  data: AggregatedData;
  currency: Currency;
  saveCurrency: (c: Currency) => void;
  cfdData: AggregatedData['cfd'];
  cfdStd: string;
  cfdLoading: boolean;
  switchStd: (key: string) => void;
  eurKrw: number;
  usdKrw: number;
  cnyKrw: number;
  ...
  return (
    <div style={{ paddingBottom: 80 }}>
        ...
        <div style={{ display: 'flex', gap: 4, marginTop: 12 }}>
          {CURRENCIES.map(c => (
            <button key={c} onClick={() => saveCurrency(c)} style={{
              ...
              color: currency === c ? '#111' : 'rgba(17,17,17,0.32)', cursor: 'pointer', fontFamily: 'inherit', minHeight: 44,
            }}>
              {CUR_SYM[c]} {CUR_LBL[c]}
            </button>
          ))}
      ...
      {/* CFD 구스다운 요약 */}
      <div style={{ padding: '14px 16px', borderBottom: '1px solid #ebebeb', display: 'flex', gap: 8 }}>
      ...
      {/* CFD 상세 */}
      <div style={{ padding: '20px 16px', borderBottom: '1px solid #ebebeb' }}>
        <MSecHdr tag="CFD · 중국우융공업협회" title="시세" sub="90% 기준" />
        ...
        <p style={{ fontSize: 9, color: 'rgba(17,17,17,0.28)', marginBottom: 10 }}>마지막 업데이트 {cfdData.updatedAt}</p>
...
function NewsTab() {
  return (
    <div style={{ padding: '20px 16px', paddingBottom: 80 }}>
      <MNewsCol title="국내 뉴스" keywords={['거위털', '오리털', '구스이불']} apiPath="/api/news-kr" sourceNote="출처: 네이버 뉴스 검색 API" />
      <Divider margin="24px 0" />
      <MNewsCol title="해외 뉴스" keywords={['거위털', '오리털', '침구류']} apiPath="/api/news" sourceNote="출처: Google News RSS" />
    </div>
  );
}
...
function ShoppingTab() {
  ...
  return (
    <div style={{ paddingBottom: 80 }}>
      <div style={{ padding: '20px 16px', borderBottom: '1px solid #ebebeb' }}>
        <MSecHdr tag="SHOPPING TREND" title="쇼핑 트렌드" sub="네이버 쇼핑" />
...
function SnsTab() {
  ...
  return (
    <div style={{ paddingBottom: 80 }}>
      <div style={{ padding: '20px 16px', borderBottom: '1px solid #ebebeb' }}>
        <MSecHdr tag="SNS INSIGHT · YouTube" title="유튜브 컨텐츠" sub="최신 영상" />
        <MYoutubeGrid />
        <p style={{ fontSize: 9, color: 'rgba(17,17,17,0.28)', marginTop: 10 }}>출처: YouTube Data API v3</p>
      </div>
      <div style={{ padding: '20px 16px' }}>
        <MSecHdr tag="NAVER SHOPPING LIVE" title="쇼핑라이브" sub="이불 · 최신" />
        <div style={{ border: '1px solid #ebebeb', overflow: 'hidden' }}>
...
export default function MobileDashboard({ data }: { data: AggregatedData }) {
  ...
  return (
    <div style={{ background: '#fff', color: '#111', minHeight: '100vh', position: 'relative' }}>
      <NoticePopup />
      ...
      <header style={{ position: 'sticky', top: 0, background: 'rgba(255,255,255,0.97)', backdropFilter: 'blur(8px)', zIndex: 50, borderBottom: '1px solid #ebebeb', height: 52, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px' }}>
        <h1 style={{ fontSize: 15, fontWeight: 400, letterSpacing: '0.12em', color: '#111' }}>GOOCHO</h1>
      ...
      <main>
        {tab === 'market' && (
          <MarketTab
            data={data} currency={currency} saveCurrency={saveCurrency}
            cfdData={cfdData} cfdStd={cfdStd} cfdLoading={cfdLoading} switchStd={switchStd}
            eurKrw={eurKrw} usdKrw={usdKrw} cnyKrw={cnyKrw} lastUpdated={lastUpdated}
          />
        )}
        {tab === 'news'     && <NewsTab />}
      ...
      <nav style={{
        ...
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{
            ...
            borderTop: tab === t.id ? `2px solid ${KEY}` : '2px solid transparent',
            marginTop: -1,
          }}>
            <span style={{ fontSize: 18 }}>{t.icon}</span>
            <span style={{ fontSize: 10, fontWeight: 700, color: tab === t.id ? KEY : 'rgba(17,17,17,0.35)' }}>{t.label}</span>
          </button>
        ))}
      </nav>

      {/* 푸터 */}
      <footer style={{ padding: '16px', borderTop: '1px solid #ebebeb', marginBottom: 'calc(60px + env(safe-area-inset-bottom))' }}>
        <p style={{ fontSize: 9, color: 'rgba(17,17,17,0.28)', lineHeight: 2 }}>
          · CFD 시세는 중국 내수 도매가 기준입니다.<br />
          · 환율은 open.er-api.com 기준이며, 실제 거래 환율과 차이가 있을 수 있습니다.<br />
          · 이 정보는 참고용이며, 투자·구매 결정의 직접 근거로 사용하지 마세요.
        </p>
        <p style={{ fontSize: 9, color: 'rgba(17,17,17,0.28)', marginTop: 8 }}>영이만을 위한 공간 © 2026</p>
      </footer>
    </div>
  );
}
